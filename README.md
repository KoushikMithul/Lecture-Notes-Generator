...
```bash
pip install python-dotenv groq transformers torch ffmpeg-python opencv-python librosa soundfile pydub python-pptx PyMuPDF
```
...
6. **Clean generated notes (optional)**
   ```python
   # Remove thinking tags from generated notes
   python clean_lecture_notes.py
   ```
   This will process all lecture notes in `data/output/lecture_notes/` and save cleaned versions in `data/output/lecture_notes/clean/`.
